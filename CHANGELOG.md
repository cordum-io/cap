...
- **Go SDK:** `ManagedWorker` now has an explicit fail-closed CAP-PRODUCTION mode: exact raw request verification and replay admission use the delivered NATS subject, while result/progress/cancel events echo the admitted identity and dispatch fence and use fresh subject-bound production signatures.

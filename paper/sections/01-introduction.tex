...
CAP was created to make those concerns part of the wire contract rather than framework-specific application logic. Its first public draft, released on December 11, 2025, already specified a versioned envelope, jobs and results, worker heartbeats, a pre-dispatch Safety Kernel, human escalation, lifecycle states, pointer-based payload separation, and workflow lineage \citep{cap_v010}. A release the next day added first-class token and deadline budgets together with tenant and principal identity \citep{cap_v200}. Cordum is the production-oriented reference implementation \citep{cordum_current,cordum_disclosure2026}.
